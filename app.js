...
const { addProduct } = require("./Controllers/imageController");
...
app.post("/create-product", uploadMultiple, addProduct);
...
app.put("/update-visibility", async (req, res) => {
  try {
    ...
    const updatedProduct = await Product.findOneAndUpdate({ productId: productId }, { $set: { visibility: visibility } }, { new: true });
...
app.delete("/product-delete/:id", async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      data: deletedProduct, // Optionally include deleted product info
    });
  } catch (error) {
...
app.get("/assign-productid", async (req, res) => {
  try {
    ...
    for (const product of products) {
      ...
      const updateResult = await Product.findOneAndUpdate({ _id: product._id }, { $set: { productId } }, { new: true });
...
app.post("/seller/send-otp", async (req, res) => {
  const { emailId } = req.body; // Extract email from request body
  const randomOtp = Math.floor(100000 + Math.random() * 900000); // Generate OTP
  console.log("emailId", emailId);

  try {
    const sellerUser = await Seller.findOneAndUpdate(
      { email: emailId }, // Match condition
      { $set: { otp: randomOtp } }, // Update OTP field
...
app.post("/seller/verify-otp", async (req, res) => {
  const { emailId, otp } = req.body; // Extract email and OTP from the request body

  try {
    // Find the seller by email
    const sellerUser = await Seller.findOne({ email: emailId });
